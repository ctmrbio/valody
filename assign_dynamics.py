...
if args.subtypes:
    ...
    if(set(all_cst) != {"I-A", "I-B", "II-A", "II-B", "III-A", "III-B", "V",\
            "IV-A", "IV-B", "IV-C0", "IV-C1", "IV-C2", "IV-C3", "IV-C4"}):
        sys.exit('When using subtypes, the  following CST must be included: \
    ...
    if(set(all_cst) != {"I", "II", "III", "IV-A", "IV-B", "IV-C", "V"}):
        sys.exit('The following CST must be included: "I", "II", "III", "IV-A", "IV-B", "IV-C", "V"')
    if(len(set(eu_cst).intersection(dys_cst))>0):
        sys.exit("A CST cannot be eubiotic and dysbiotic at once")    
